...
class SizeCheck(FileCheck):
    def can_check(self) -> bool:
        return (
            not self.src_path.endswith(".dcm")
    ...
    def check(self) -> bool:
        ...
        info = {"src_size": src_size, "ref_size": ref_size, "diff_size": diff_size}

        # report facts
        if src_size > ref_size:
            self.add_finding(
                "Size Difference", "file size is larger than reference", info
            )
        elif src_size < ref_size:
            self.add_finding(
                "Size Difference", "file size is smaller than reference", info
            )
