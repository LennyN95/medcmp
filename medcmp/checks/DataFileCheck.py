...
def compare_numbers(
    v1: Union[int, float], v2: Union[int, float], verbose: bool = False
):
            ...
            if round_down(v1, precision) == round_down(v2, precision):
                if verbose:
                    print(
                        ...
                        precision,
                        "(",
                        round_down(v1, precision),
                        "-",
                        round_down(v2, precision),
                        ")",
                    )
...
def check_item(item: ComparisonItem):
    ...
    if (isinstance(v1, int) or isinstance(v1, float)) and not isinstance(v1, bool):
        ...
        if match:
            ...
            item.info = {"scale": scale, "precision": precision, "src": v1, "ref": v2}
        else:
            item.outcome = ComparisonOutcome.VALUE_MISMATCH
            item.info = {"src": v1, "ref": v2}
...
class DataFileCheck(FileCheck):
    ...
    def check(self):
        ...
        # overview (notes)
        exact_value_paths = [
            item.path for item in items if item.outcome == ComparisonOutcome.VALUE_EXACT
        ]
        exact_value_paths_str = (
            ",".join(exact_value_paths)
            if len(exact_value_paths) < 20
            else ",".join(exact_value_paths[:20])
            + " (+ "
            + str(len(exact_value_paths) - 20)
            + " more)"
        )
        if len(exact_value_paths) > 0:
            self.add_note(
                "Value Match", "These keys have identical values", exact_value_paths_str
            )
        ...
        for item in items:
            ...
            if item.outcome == ComparisonOutcome.MISSING:
                self.add_finding(
                    "Missing key",
                    f"Key '{item.path}' is missing in source file",
                    item.info,
                    subpath=item.path,
                )
                ...
                self.add_finding(
                    "Extra key",
                    f"Key '{item.path}' is extra in source file",
                    item.info,
                    subpath=item.path,
                )
                ...
                self.add_finding(
                    "Type mismatch",
                    f"Type of key '{item.path}' is different in source file",
                    item.info,
                    subpath=item.path,
                )
                ...
                self.add_finding(
                    "Value mismatch",
                    f"Value of key '{item.path}' is different in source file",
                    item.info,
                    subpath=item.path,
                )
                ...
                self.add_finding(
                    "Value similar",
                    f"Value of key '{item.path}' is similar in source file",
                    item.info,
                    subpath=item.path,
                )
