...
class ImageFileCheck(FileCheck):
    ...
    def check(self) -> bool:
        ...
        self.add_note(
            "Data Type", "Data type of the reference image", str(ref_np.dtype)
        )
    ...
    def check_values(self) -> bool:
        ...
        self.add_note(
            "Image Diff Stat",
            "Statistics of the diff image between src and ref.",
            diff_img,
        )

        # check that MIN and MAX are within tolerance
        check = bool(
            np.isclose(diff_img["MIN"], 0.0, atol=self.value_tolerance)
        ...
        if not check:
            self.add_finding(
                "Image Diff Stat",
                "Difference between reference and test image exceeds tolerance.",
                {
                    "MIN": diff_img["MIN"],
                    "MAX": diff_img["MAX"],
    ...
    def check_dice(self) -> bool:
        ...
        # add note
        self.add_note(
            "Dice Score", "Dice score between reference and test image", dice_score
        )
        ...
        if not check:
            self.add_finding(
                ...
                "Dice score between reference and test image exceeds tolerance.",
                {"dice_score": dice_score, "tolerance": self.dice_tolerance},
            )
...
def pypla_compare(
    path_to_reference_img, path_to_test_img, verbose=True
) -> Dict[str, float]:
    """
