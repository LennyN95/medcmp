...
def create_file(
    base: str,
    path: str,
    src_content: Optional[Union[dict, str]],
    ref_content: Optional[Union[dict, str]],
):
...
class ReportPassingTest(unittest.TestCase):
    ...
    def test_report(self):
        ...
        self.assertEqual(
            report_yml_data["summary"]["checks"]["DataFileCheck"]["files"], 2
        )
        self.assertFalse(
            "findings" in report_yml_data["summary"]["checks"]["DataFileCheck"]
        )
        self.assertEqual(report_yml_data["summary"]["checks"]["SizeCheck"]["files"], 4)
        self.assertFalse(
            "findings" in report_yml_data["summary"]["checks"]["SizeCheck"]
        )
...
class ReportDeviationTest(unittest.TestCase):
    def setUp(self):
        ...
        # create temporary structure
        create_file(
            self.base, "data.yml", {"a": 1, "b": 2.00000}, {"a": 1, "b": 2.00008}
        )
        create_file(
            self.base,
            "deep/data.json",
    ...
    def test_report(self):
        ...
        self.assertListEqual(
            report_yml_data["missing_files"], ["deep/nested/missing.yml"]
        )
        ...
        self.assertDictEqual(
            report_yml_data["summary"]["checks"]["DataFileCheck"]["findings"],
            {"Value similar": 1, "Extra key": 1, "Missing key": 1},
        )
        self.assertEqual(report_yml_data["summary"]["checks"]["SizeCheck"]["files"], 4)
        self.assertDictEqual(
            report_yml_data["summary"]["checks"]["SizeCheck"]["findings"],
            {"Size Difference": 2},
        )
